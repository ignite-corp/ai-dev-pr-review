...
1~3 은 §5 를 준비하며 스스로 찾았고(3건), **4·5·6·7 은 #173 의 첫 라운드가**(4건),
**8~14 는 두 번째 라운드가**(7건, 8번은 리뷰어의 시간 불일치 지적을 따라가다 드러남) 찾았습니다.
**자기 점검 3 : 외부 리뷰 11.** 라운드가 돌수록 **외부 발견 비율이 오르고 있고**, 그 자체가
*"자기 검증은 자기가 밟은 경로만 증명한다"* 는 이 문서의 주장에 대한 증거입니다.
열넷 모두 이 문서가 **서술하는 부류**에 속합니다 — 그래서 지우지 않습니다.
...
> `ThreadPoolExecutor(max_workers=3)` 에 셋을 한꺼번에 넣는다. 즉 **직렬화는 #170 에만 구현돼 있고**,
...
> `ThreadPoolExecutor(max_workers=3)` 로 셋을 동시에 띄운다(§5-0). 결과는 §1-11 이 닫으려던 바로 그
...
| 6 | **리뷰어 직렬화 자체** — #170 은 루프, #172 는 `ThreadPoolExecutor(max_workers=3)` | #170 에만 — **§5 의 실행이 찾았다**; 리뷰 열한 라운드도, 두 축 감사도, #172 본문의 표도 못 봄. 게다가 **#172 본문은 이 성질을 자기 것이라고 적는다** |
