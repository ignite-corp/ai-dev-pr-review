...
읽은 범위(전부, 표본 아님). **스냅숏 시점은 2026-09-20, §5 의 실행 직전**입니다 — 그 실행이 #170 에
코멘트 둘을 더해 지금은 24개이고(§5-8), 그중 하나를 §5-3 이 증거로 인용합니다. 아래 22 는 **리뷰
기록으로 읽은 범위**이고, 실행이 만든 둘은 §5 에서 따로 다룹니다:
...
§5 의 숫자는 요약을 옮긴 것이 아니라 원자료에서 **다시 측정**한 것이고, **재현 가능한 근거는
`evidence/` 에 함께 커밋했습니다** — 드라이버 출력 전문, 세 리뷰어의 판정과 로그, 실행 스크립트,
파생값을 전부 다시 계산하는 `evidence/scripts/measure.py`, 그리고 부피 때문에 넣지 않은 큰 파일들의
크기·sha256 표(`evidence/MANIFEST.md`). 본문이 `/tmp/lens-harvest/` 를 가리키는 자리는 **휘발성
원본**을 뜻하며 참조가 아닙니다 — 그 경로는 재부팅에 사라집니다.
...
|---|---|---|---|
| 1 | 열지도 않은 출처(Jira)에 대해 **티켓 부재를 단언** — §3-2 의 셋 중 **둘**(pyright→AT-2418, 80줄→AT-2420) 과 §3-3 의 둘 중 **하나**(Normalize→AT-2424), 합쳐 **다섯 중 셋**이 틀림 | §4-A | §3-2 · §3-3 |
...
| 7 | §6 의 권고가 §3-6 의 **일곱 중 다섯만** 이름함(2-E · 2-I 누락) — 재작성이 둘을 조용히 상속. *(§2-J 는 §5 이후 추가돼 당시 양쪽 어디에도 없었고, 재계수에서 함께 넣어 지금은 여덟이다)* | §4-A | §6-9 |
| 8 | **§5 의 실행을 "직렬로 돌았다"고 적음** — 실제로는 `parallel`, 셋이 동시에. mtime 순서를 근거로 썼는데 **그 검사는 직렬과 병렬을 구분하지 못한다** | §4-A(10번 행) | §5-0 · §1-11 |
| 9 | §4-G 가 같은 사건을 **"두 번"** 이라 쓰고 값 **셋**을 나열 — §5-0 은 같은 사건을 "세 번"이라 적음 | §4-A | §4-G |
| 10 | §5-2 의 결론(*"한도를 지킨 더 큰 PR 이면 닿는다"*)을 **같은 문장의 숫자가 반박** — 한도가 3000 인데 측정된 PR 이 6109줄 | §4-B | §5-2 |
| 11 | §2-C 의 **"약 244 KB" 추정을 이 실행이 83,579 B 로 대체**했는데 그 사실을 적지 않음 | §4-F | §2-C · §5-2 |
| 12 | 같은 파일이 §5-1 에서 1124줄, §5-3 에서 1687줄 — **표에 어느 빌드인지 표시 없음** | §4-A | §5-1 · §5-3 |
| 13 | §5-8 제목이 **"아무것도 접히지 않았다"** 인데 잰 것은 이슈 코멘트 minimize 뿐이고, §5-3 에는 "접힘"이 있음 | §4-B | §5-8 |
| 14 | §1-7 의 페이로드를 무력화하면서 **§1-8 의 쌍둥이를 남김** | §4-D | §1-8 |
| 15 | **§5 의 증거 전부가 `/tmp` 한 경로에 걸려 있었음** — 재부팅하면 41/1 도 83,579 B 도 sha256 도 재확인 불가. *"요약을 출력 대신 읽지 마라"* 를 **독자가 이 문서에 적용할 수 없는 상태** | §4-A | `evidence/` · §5-0 |
| 16 | 같은 호출을 `max_workers=3` 과 `max_workers=len(REVIEWER_NAMES)` **두 형태로 인용** — 값이 같은 것은 `REVIEWER_NAMES` 가 지금 셋이기 때문일 뿐 | §4-A | §5-0 · §3-1 · §1-11 · §4-C |
| 17 | 권고 6 의 *"§4-D 의 일곱 **전부**"* 를 **자기 표가 반박** — 둘은 지적된 자리가 아예 없었다 | §4-B | §6-6 |
| 18 | 표 7행의 뺄셈(여덟−다섯=둘)이 안 맞음 — 수정 전 프레임은 **일곱 중 다섯** | §4-A | 이 표 7행 |
| 19 | 표 1행의 "다섯 중 셋"이 §3-2 만 가리킴 — 분모는 §3-2 와 §3-3 을 합친 것 | §4-A | 이 표 1행 |
| 20 | §3-6 의 여덟에서 **2-F 만 빠진 이유가 없었음**(2-B · 2-K 는 적혀 있었다) | §4-A | §3-6 |

**출처별로 보면 이 표가 §4-A 의 논지에 대한 데이터입니다.**
1~3 은 §5 를 준비하며 스스로 찾았고(3건), **4~7 은 #173 의 첫 라운드**(4건),
**8~14 는 두 번째 라운드**(7건, 8번은 리뷰어의 시간 불일치 지적을 따라가다 드러남),
**15~20 은 세 번째 라운드**(6건)가 찾았습니다.
**자기 점검 3 : 외부 리뷰 17.** 세 라운드의 외부 발견은 4 → 7 → 6 이고 **자기 점검은 첫 회 이후 0** 입니다.
스스로 *"모든 수를 다시 세라"* 를 규율로 적어 놓고도 매 라운드 새 오류가 외부에서 나옵니다 —
그 비율 자체가 *"자기 검증은 자기가 밟은 경로만 증명한다"* 에 대한 증거입니다.
스물 모두 이 문서가 **서술하는 부류**에 속합니다 — 그래서 지우지 않습니다.
...
BEFORE  리뷰어 시작 시 review-claude.json 존재: True
          summary: <PR 이 심은 "문제 없음" 요약 — 여기서는 생략>   early_exit: True
        has_early_exit -> True (체인을 끊음)
        집계가 "수행된 리뷰"로 읽음: ok
AFTER   존재: False / has_early_exit -> False / 집계 상태: failed
```

> **§1-7 의 쌍둥이였다.** 앞 라운드에서 §1-7 의 적대적 지시문을 무력화하면서 **여기 같은 모양을
> 그대로 두었다** — §4-D 가 목록화한 *"한 부류의 한 인스턴스만 고치고 형제를 남긴다"* 를, 그 부류를
> 적어 둔 문서가 저질렀다. #173 리뷰어가 찾았다. 증거는 심어진 요약 **문구**가 아니라
> `early_exit: True` 가 체인을 끊었다는 것과 집계가 그것을 `ok` 로 읽었다는 것이므로, 그 둘은 남기고
> 문구만 줄인다. **문서 전체를 다시 훑어 남은 페이로드가 없음을 확인했다** — `.git/config` 증거는
> 이미 `sh -c 'touch ...'` 로 줄어 있고, `PLANTED HOOK RAN` · `PLANTED BY REVIEWER 1` 은 프로브가
> 심은 **표지 문자열**이지 모델에게 말을 거는 지시문이 아니다.
...
> **이 불변식은 한 번도 실행으로 시험된 적이 없다 — §5 의 실행도 시험하지 않았다.**
> §5 가 돌린 빌드는 #172 의 head 이고, 거기 `run_reviewers` 는 비-sequential 경로에서
> `ThreadPoolExecutor(max_workers=len(REVIEWER_NAMES))` 에 리뷰어 전원을 한꺼번에 넣는다
> (`REVIEWER_NAMES` 는 현재 셋). 즉 **직렬화는 #170 에만 구현돼 있고**,
> 실행은 기본값 `parallel` 로 **실제 동시 실행**됐다(산술과 코드 근거는 §5-0).
> 이 결정을 받치는 것은 **#170 의 단위 테스트**(일부러 다시 동시로 만든 빌드에 걸어 실패를 확인한 것)
> 뿐이고, **끝까지 도는 실행에서 확인된 적은 없다.** 재작성의 반드시-측정 목록(§6-12)에 올린다.
> 그리고 **#172 의 본문은 이 성질을 자기 것이라고 적는다** — 그 문장은 그 브랜치에 대해 거짓이다(§4-C 6번).
...
- **측정됨.**
  ...
  > **⚠ 이 "약 244 KB" 는 추정이었고, §5-2 의 실측이 대체했다.** 한도의 두 배가 넘는 6109줄 PR 에서
  > 실제 프롬프트는 **83,579 B**(추정의 약 1/3). 추정이 **diff 비례**를 가정했으나 측정해 보니
  > **프롬프트에 diff 가 들어 있지 않다** — 값이 아니라 모형이 틀렸다. 위험 자체는 남지만 축이
  > 다르다: 프롬프트를 키우는 것은 PR 크기가 아니라 **누적 맥락과 스레드 수**이고, 그쪽은
  > `PR_SIZE_LIMIT` 이 묶지 않는다. **이 줄은 근거로 인용하지 말 것 — §5-2 를 인용할 것.**
...
### 3-1. #172 본문의 "Merge constraint" 다섯 — **표에 적힌 다섯은 정확하다. 표는 완전하지 않다.**
...
> **표는 다섯 줄이고, 여섯째가 있다 — §5 가 찾았다.**
> **`run_reviewers` 의 직렬화 자체**가 #170 에만 있다: #170 은 루프로 하나씩 돌리고, #172 는
> `ThreadPoolExecutor(max_workers=len(REVIEWER_NAMES))` 로 리뷰어 전원을 동시에 띄운다(§5-0).
> 결과는 §1-11 이 닫으려던 바로 그
> 경합 — 두 리뷰어가 같은 트리에 동시에 쓰는 것 — 이 **#172 단독 머지 시 그대로 열린다.**
>
> **이것이 5행과 같은 함수 안에 있다는 점이 중요하다.** 표의 5행은 이미 `run_reviewers` 를 이름하면서
> **결론 딕셔너리 소유권**만 보고했다. 즉 두 축 감사는 그 함수를 **열어 보고도** 더 큰 차이를 지나쳤다 —
> 그러니 이 누락의 원인은 위 문장이 말하는 *"다른 파일이라 못 봤다"* 가 아니라,
> **한 함수에서 차이를 하나 찾고 멈춘 것**(§4-D)이다. 본문 표가 *"검증했다. 표는 정확하다"* 인 이 절의
> 제목도 그래서 좁혀 읽어야 한다 — **표에 적힌 다섯은 정확하고, 표가 완전하지는 않다.**
...
**§2 의 열한 항목 중 셋이 이 목록에 없고, 셋 다 이유가 있다:**

- **2-B**(리뷰어가 삭제 대상을 다시 만들면) — worktree 재작성으로 **질문 자체가 사라졌다.**
  재작성이 다시 "지우고 리뷰어가 다시 만들면?"을 만나면 그때 돌아온다(§2-B 에 그렇게 적었다).
- **2-F**(`ConfigError` 를 판정으로 게시할 것인가) — **결정이 났다.** 게시하지 않고 `::error::` +
  종료 코드 1 이며, 근거(집계가 해석 못 한 바로 그 설정을 필요로 한다)가 §2-F 에 적혀 있다.
  다만 그 결정이 *"한 실행은 판정을 낸다"* 에 구멍 하나를 열어 두고 **그 구멍의 경계가 라운드마다
  움직였으므로**, 재작성은 경계를 처음부터 정해야 한다 — **권고 2 에 접어 두었다**(불변식과 그
  예외 조항을 함께 세우는 항목).
- **2-K**(타입 체크 · 함수 길이) — 티켓이 있어 §3-2 · 권고 4 에서 따로 다룬다.

**여덟 다 §6 의 권고 9 가 이름한다** — 그 둘의 수가 어긋나면 재작성이 조용히 물려받는다.
*(2-E · 2-I 가 실제로 그렇게 빠져 있었고, #173 리뷰어가 두 수를 맞대어 찾았다. 그때 2-B · 2-K 는
빠진 이유가 적혀 있었는데 **2-F 만 아무 말 없이 빠져 있었다** — 같은 조용한 상속의 모양이라
이번에 함께 적는다.)*
...
| 10 | **이 문서가**, 리뷰어 로그의 **mtime 순서**를 보고 "직렬로 돌았다"고 적음 — mtime 순서는 직렬에서나 병렬에서나 **똑같이 정렬돼 나오므로 둘을 구분할 변별력이 0** 이다. 원자료에 `parallel` 이라고 적혀 있었고 코드에 `ThreadPoolExecutor` 가 있었다 | §5-0 |
...
> **그리고 그 다음 라운드가 10번 행을 더했다** — 이 문서 자신이 저지른 두 번째 §4-A 사례이고,
> 이번 것은 *수를 안 센 것*이 아니라 **구분할 수 없는 검사를 구분에 쓴 것**이다.
...
### 4-C. **스택 위쪽 PR 에서 고치고 아래쪽에 없다** — 아래쪽이 먼저 머지된다. **여섯 번.**
...
| 6 | **리뷰어 직렬화 자체** — #170 은 루프, #172 는 `ThreadPoolExecutor(max_workers=len(REVIEWER_NAMES))` | #170 에만 — **§5 의 실행이 찾았다**; 리뷰 열한 라운드도, 두 축 감사도, #172 본문의 표도 못 봄. 게다가 **#172 본문은 이 성질을 자기 것이라고 적는다** |
...
**그 method 로도 여섯 번째는 못 찾았다 — 같은 파일, 같은 함수 안인데도.** 6번(직렬화)은 축 (2) 가
정확히 겨냥한 모양인데, 두 축 감사는 `run_reviewers` 를 열어 **차이 하나를 찾고 멈췄다.**
그래서 이 부류의 교훈은 *"축을 잘 고르라"* 가 아니라 하나 더 있다 — **한 함수에서 차이를 하나 찾으면
그 함수를 다 본 것처럼 다음으로 넘어간다**(§4-D). 결국 이것을 드러낸 것은 감사도 리뷰도 아니고
**코드를 한 번 돌려 본 것**이다(§5-0).
...
- **PR 이 세 번 크기 한도를 넘어 리뷰가 스킵됐다**(3291·3196·3007줄 / 한도 3000 — #170 이 두 번,
  #172 가 한 번). 매번 자기 도구가
...
아래 숫자는 전부 원자료에서 **직접 다시 측정**했다. 원자료는 실행 당시 `/tmp/lens-harvest/` 에
있었고 — **휘발성이다. 재부팅에 사라진다** — 그래서 이 절이 기대는 부분을 `evidence/` 에
**함께 커밋했다**: 드라이버 출력 전문 · 세 판정 파일과 리뷰어 로그 · 실행 스크립트 ·
계측 오류의 대조 실험 · 파생값을 다시 계산하는 `evidence/scripts/measure.py`.
커밋하지 않은 큰 파일(`pr.diff` 257 KB · `codex-run.log` 540 KB 등)은
`evidence/MANIFEST.md` 에 **크기와 sha256, 그리고 그것을 잰 명령**으로 남겼다.
`measure.py` 는 인자 없이도 *발동한 가드 수*(경고 0 · 오류 1)를 커밋된 로그만으로 다시 낸다.
...
| 실행 방식 | `REVIEW_MODE` 기본값 `parallel` — **셋이 동시에 돌았다**(아래 정정) |
| 완료 순서 | gemini 21:31:33 · codex 21:35:46 · claude 21:37:17 — mtime 은 **시작이 아니라 완료** 순서 |
...
**정정 — 초안은 이 실행이 §1-11 의 "한 번에 하나"를 확인했다고 적었다. 정반대다.**
#173 리뷰어가 *"직렬이라면 claude 의 창은 ~120초인데 `duration_api_ms` 가 380.7초"* 라고 짚었고,
확인해 보니 **불일치의 원인은 계측이 아니라 이 문서의 주장이었다.** 산술로 결판난다:

```
claude 종료      21:37:17.99      claude duration_ms 380,819  ->  시작 21:30:57.2
gemini 종료      21:31:33.73      <- claude 가 이미 36초째 돌고 있던 시점
개별 소요 합     36 + 289 + 381 = 706초        벽시계(pr.diff 기록~종료) 387초
합 > 벽시계  ->  직렬일 수 없다
```

코드가 그것을 확정한다. **실행된 #172 빌드의 `run_reviewers` 는 비-sequential 경로에서
`ThreadPoolExecutor(max_workers=len(REVIEWER_NAMES))` 에 셋을 한꺼번에 `submit` 한다.**
#170 빌드에는 `ThreadPoolExecutor` 가 **0회** 등장하고 docstring 이 *"One reviewer's process group at
a time, in either mode"* 라고 적는다. 즉 **직렬화는 #170 에만 있다.**

그러므로 **이 실행은 §1-11 의 불변식을 시험하지 않았다** — 그 불변식이 구현돼 있지 않은 빌드를,
그것이 적용되지 않는 `parallel` 모드로 돌렸다. §5-6 에서 격리를 두고 한 정정과 **같은 모양**이다:
*완화책이 있는데도 일어난 것이 아니라, 완화책이 없는 빌드에서 일어난 것.*

**그리고 이것이 §4-C 의 여섯 번째 사례다.** 직렬화는 #170 에 있고 #172 에 없는데,
**#172 의 본문은 "The reviewers always run one at a time" 이라고 적는다** — 자기 코드가 부정하는
문장이다(§4-B 의 모양이기도 하다). **#172 본문의 다섯 줄짜리 머지 제약 표도, 저자의 두 축 감사도
이것을 잡지 못했다** — 표의 5행이 `run_reviewers` 를 이미 이름하면서 그 함수의 **다른** 차이
(결론 딕셔너리 소유권)만 보고했다. 같은 함수 본문 안의 더 큰 차이를 놓친 것이므로 §4-D 이기도 하다.
§3-1 에 덧붙였다.
...
**세는 대상은 실제로 돌아간 빌드 — #172 head `e9592d8`(§5-0 의 sha256 `217da94…`)** 이다:

| 모듈 (#172 head `e9592d8`) | `print(::warning::)` | `raise DriverError(...)` | 줄 수 |
|---|---|---|---|
...
argv 쪽은 **한도의 3분의 2 를 쓰고 있다.** 다만 **초안이 여기서 끌어낸 결론은 이 숫자가 반박한다.**
초안은 *"한도를 지킨 더 큰 PR 이면 닿는다"* 고 적었는데, **이 PR 이 6109줄이고 기본 한도가 3000 이므로
한도를 지킨 PR 은 이것보다 클 수 없다.** 그런 PR 은 존재하지 않는다. #173 리뷰어가 같은 문장 안의
두 수를 맞대어 찾았다.

**그리고 무엇이 프롬프트를 키우는지가 측정으로 뒤집혔다.** 구성을 쟀다:

```
codex-prompt.md   83,579 B  =  context.md 63,694 B  +  19,885 B(스레드 블록 + review_prompt.md)
pr.diff           257,081 B  ->  codex-prompt.md 안에 들어 있지 않음 (확인: 부분 문자열 검색 False)
```

**diff 는 프롬프트에 들어가지 않는다** — CLI 가 작업 트리에서 직접 읽는다. 그러므로 argv 압력은
**PR 크기가 아니라 누적된 리뷰 맥락과 스레드 수**가 만들고, **`PR_SIZE_LIMIT` 은 그것을 전혀 묶지
않는다.** 위험은 실재하되 초안이 지목한 축이 틀렸다: 한도를 올려도 안 닿고, 한도를 지켜도 스레드가
쌓이면 닿는다.

> **§2-C 의 "약 244 KB" 추정은 이 실행이 대체했다.** 그 수는 리뷰 당시의 추정이었고, 실측은
> **83,579 B** — 한도의 **두 배가 넘는 PR** 에서 추정의 약 1/3 이다. 추정이 diff 비례를 가정했는데
> 프롬프트가 diff 를 담지 않으므로, 틀린 것은 값만이 아니라 **모형**이었다. §2-C 에 그렇게 달았다.

두 가드는 **발동하지 않았지만 추측이 아니었다** — 그 점은 그대로 유효하다. — **측정됨**

### 5-3. **정작 일어난 실패는 가드 목록 어디에도 없다** — 줄 번호

세 리뷰어가 낸 4건 중, PR 에 **인라인으로 닿은 것은 1건**이다. 나머지가 어떻게 됐는지가 이 절의 핵심이다.

**여기의 "실제" 줄 수는 리뷰 *대상*인 #170 head `ce8c324` 기준이다** — §5-1 표의 줄 수는
돌아간 #172 빌드 기준이므로 같은 파일 이름이 두 길이를 갖는다(1124 대 1687). 두 빌드가 다르다는
사실 자체가 §5-0 의 요점이다.

| 리뷰어 | 지목한 위치 | 실제 (#170 head `ce8c324`) | 결말 |
|---|---|---|---|
| claude #1 | `review_pr_local.py:2268` | 파일은 **1687줄** — EOF 밖 | 범위 검사가 버림 |
| claude #2 | `review_codex_local.py:654` | 파일은 **365줄** — EOF 밖 | 범위 검사가 버림 |
| gemini | `review_pr_local.py:1240` | 범위 **안**, 그러나 틀린 줄 | 범위 검사 통과 → 인라인 대신 **R13 컷오프 요약 코멘트로 흡수**(기존 코멘트를 minimize 한 것이 아니다 — §5-8 과 다른 뜻) |
...
### 5-8. `BOT_LOGIN` 이 운영자라 **이전 이슈 코멘트가 하나도 minimize 되지 않았다**
...
덧붙여 두 코멘트 모두 `## [bot] …` 라는 제목을 달고 **사람 계정에서** 나간다.

> **잰 범위를 제목에 적는다.** 이 측정은 **이슈 코멘트의 minimize 상태**만 본다 — 인라인 리뷰
> 스레드의 resolve 여부나 리뷰 본문은 재지 않았다. 초안 제목은 *"아무것도 접히지 않았다"* 였는데,
> **§5-3 에는 gemini 의 지적이 "R13 컷오프로 접힘"이라고 적혀 있어** 두 절이 모순으로 읽혔다.
> 두 "접힘"은 다른 일이다: 여기서는 **기존 코멘트를 minimize** 하는 것이고, §5-3 에서는
> **새 지적을 컷오프 요약 코멘트 하나로 흡수**하는 것이다. 제목을 잰 것으로 좁히고 §5-3 에도
> 그 구분을 달았다. **범위 없는 총칭을 제목에 쓴 것**이므로 §4-B 의 부류이고, #173 리뷰어가 찾았다.
> — **측정됨**(이슈 코멘트 minimize) / **미측정**(스레드 resolve · 리뷰 본문)
...
1. **스택으로 나누지 말 것.** §4-C 가 **여섯 번** 났다 — 여섯 다 위쪽에서 고쳐져 아래쪽에 없었고,
   그중 **셋**은 아래쪽 PR 에서 "Fixed" 로 공개 답변까지 됐으며(그 답은 작업한 브랜치에 대해서만 참).
   출처별로는 **넷**을 저자의 두 축 감사가, **하나**를 리뷰어가, **여섯째를 §5 의 실행이** 찾았다 —
   **감사도 리뷰도 못 찾은 것이 한 번 돌려 보니 나왔다**는 것이 이 항목의 무게다. 나눠야 한다면 **아래쪽부터 고치고 위로 리베이스**하거나,
...
6. **결함을 고칠 때 형제를 같은 목록으로 훑을 것.** §4-D 의 일곱 중 **다섯**이 "지적된 하나 +
   안 지적된 n" 이다 — 두 shim 을 나란히 놓는 것만으로 셋이 더 나왔다.
   **나머지 둘은 더 나쁜 쪽이라 따로 읽어야 한다**: `맨 subprocess.run`(R12 에 넷) 과
   `정적 분석 경고 묶음`(두 번)은 **지적된 자리가 애초에 없었다.** 앞의 다섯은 지적 하나를 실마리로
   훑으면 나오지만, 이 둘은 **훑을 실마리조차 없었고** — 도구가 이미 띄운 경고를 묶음의 하나만
   확인하고 넘긴 경우다. 그러니 권고는 둘로 갈린다: *지적을 받으면 형제를 훑을 것*, 그리고
   ***도구가 낸 경고 묶음은 항목마다 확인할 것*** — 하나를 검증하고 나머지로 일반화하지 말 것.
   *(초안은 "일곱 전부"라고 적었다. 자기 표가 반박하는 총칭이었고 — §4-B 의 모양 — #173 리뷰어가 찾았다.)*
