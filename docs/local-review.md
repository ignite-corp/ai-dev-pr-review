...
| `codex` | the Codex reviewer | `codex login` beforehand, **or** `OPENAI_API_KEY` in the environment; the driver never logs you in, so it cannot clobber your `~/.codex/auth.json` |
| `GOOGLE_AI_API_KEY` | the Gemini reviewer | read from the environment; there is no secret store here, so the driver passes your own environment through |

Two of the three reviewers can take a key from the environment --
`GOOGLE_AI_API_KEY` for Gemini and `OPENAI_API_KEY` for Codex, the latter
only if you have not run `codex login`. The driver sets neither: on Actions
these come from the org's secrets, and locally whatever you have exported is
...
The run directory is part of that surface, not just the checked-out files.
The reviewer CLIs run inside it with write access (`codex exec --sandbox
workspace-write` may write anywhere in the workspace) and the artifact
cleanup deliberately leaves `.git` alone, so what one run writes there would
still be there for the next one -- including the config keys git executes by
itself: a planted `filter.<driver>.smudge` runs on the driver's own
`git checkout`, and `core.fsmonitor` on its `git fetch`, as you, with the
clone's credential helper already configured.

The driver therefore **reuses the clone only when it is the one it left
there**: it writes a fingerprint of `.git/config` beside the clone at the
end of each run and re-reads it at the start of the next, and a directory
whose config has changed since -- or that the driver did not create -- is
deleted and cloned again. Unsetting the dangerous keys one by one would be
an allow-list by another name; this asks whether the file changed at all.
`core.hooksPath` is still set to `/dev/null` and `.git/hooks` still deleted
every run, because that is cheap and does not depend on the fingerprint.

What that does **not** cover: the rest of `.git` (objects, refs, `info/`,
alternates) is not fingerprinted, and the work tree is not either -- the
reviewers are supposed to write there. If you have reviewed something you
have reason to distrust, deleting the run directory
(`rm -rf ~/.cache/lens/local-review/<owner>-<repo>-pr<n>`) is still the
thorough answer.
