...
@pytest.mark.parametrize(
    "failure",
    [
        pytest.param(subprocess.TimeoutExpired("git", 600), id="timeout"),
        pytest.param(subprocess.SubprocessError("broken pipe"), id="subprocess-error"),
        pytest.param(OSError("not on PATH"), id="oserror"),
    ],
)
def test_run_converts_every_failure_it_can_produce(monkeypatch, failure):
    """A bounded call still raises; the bound was never the guard.

    Everything in this module goes through run(), so anything it lets
    through escapes main()'s DriverError handler and ends the run in a
    traceback with no verdict -- the escape clone_origin's own comment
    describes, in the helper clone_origin's callers use.
    """

    def raising(*a, **k):
        raise failure

    monkeypatch.setattr(review_pr_local.subprocess, "run", raising)
    with pytest.raises(review_pr_local.DriverError):
        review_pr_local.run(["git", "status"])
...
def test_a_committed_symlink_does_not_stop_the_run(tmp_path):
    """`is_dir()` follows a link, and rmtree refuses one.

    A PR that commits `.review-context` as a directory symlink made the
    directory branch hand rmtree a link, and the raise happened in prepare --
    before any reviewer, before the aggregate -- so that PR got no verdict at
    all. The link is removed; what it points at is not ours to delete.
    """
    target = tmp_path / "somewhere-else"
    target.mkdir()
    (target / "keep.txt").write_text("not ours to delete\n", encoding="utf-8")
    work = tmp_path / "repo"
    work.mkdir()
    (work / ".review-context").symlink_to(target, target_is_directory=True)
    (work / "pr.diff").write_text("stale\n", encoding="utf-8")

    review_pr_local.clean_artifacts(work)

    assert not (work / ".review-context").is_symlink()
    assert not (work / "pr.diff").exists()
    assert (target / "keep.txt").exists(), "the link's target is not ours"


def test_a_file_symlink_is_removed_not_followed(tmp_path):
    """The same for the file names: unlink the link, leave the target."""
    target = tmp_path / "elsewhere.json"
    target.write_text("{}", encoding="utf-8")
    work = tmp_path / "repo"
    work.mkdir()
    (work / "review-claude.json").symlink_to(target)

    review_pr_local.clean_artifacts(work)

    assert not (work / "review-claude.json").is_symlink()
    assert target.exists()


def test_a_cleanup_failure_is_a_named_error_not_a_traceback(tmp_path, monkeypatch):
    """prepare runs before the aggregate, so an escape here costs the verdict."""
    work = tmp_path / "repo"
    work.mkdir()
    (work / "pr.diff").write_text("x", encoding="utf-8")

    def denied(self, *a, **k):
        raise OSError(13, "Permission denied")

    monkeypatch.setattr(Path, "unlink", denied)
    with pytest.raises(review_pr_local.DriverError, match="cannot clear"):
        review_pr_local.clean_artifacts(work)
...
def _trust(work: Path) -> None:
    """Mark a hand-made clone as one this driver left, so it is reused.

    Without it ensure_clone replaces the directory, which is the point of
    the marker: a clone whose config this driver did not write is not one it
    will run git commands in.
    """
    (work.parent / review_pr_local.CLONE_MARKER).write_text(
        review_pr_local.config_fingerprint(work), encoding="utf-8"
    )
...
def test_a_run_directory_for_another_repository_is_refused(tmp_path, monkeypatch):
    ...
    # Trusted, so the marker does not replace it first: this is the case the
    # identity check is still for -- the same run directory pointed at a
    # different repository, by a driver that made the clone itself.
    _trust(work)
...
def _stub_clone(monkeypatch, origin: Path, counter: list[str]) -> None:
    """`gh repo clone` without a network, counting how often it is called."""
    real = review_pr_local.run

    def fake(argv, **kwargs):
        if argv[:3] == ["gh", "repo", "clone"]:
            counter.append(argv[4])
            subprocess.run(
                ["git", "clone", "-q", str(origin), argv[4]],
                check=True,
                capture_output=True,
            )
    ...
    monkeypatch.setattr(review_pr_local, "run", fake)


def test_a_clone_whose_config_changed_is_replaced(tmp_path, monkeypatch):
    """The config keys git executes are not a list to unset; they are a reason
    to distrust the directory.

    disarm_hooks closes `.git/hooks`. What it cannot close is the keys git
    runs itself -- a planted `filter.<drv>.smudge` executes on the driver's
    own `git checkout --force --detach` -- and the transport redirects. The
    marker beside the clone is what makes the reuse conditional at all.
    """
    ...
    monkeypatch.setenv("GIT_TERMINAL_PROMPT", "0")
    _identity_check_off(monkeypatch)
    _stub_clone(monkeypatch, origin, cloned)

    review_pr_local.ensure_clone(work, "o/r")
    assert len(cloned) == 1, "nothing cached yet"
    review_pr_local.ensure_clone(work, "o/r")
    assert len(cloned) == 1, "an untouched clone is reused, not re-cloned"

    _git(work, "config", "--local", "filter.lens.smudge", "touch /tmp/pwned")
    review_pr_local.ensure_clone(work, "o/r")
    assert len(cloned) == 2, "a changed config replaces the clone"
    assert not _git(
        work, "config", "--local", "--default", "", "--get", "filter.lens.smudge"
    )


def test_a_fetch_does_not_make_the_clone_untrusted(tmp_path, monkeypatch):
    """Reuse has to survive the driver's own git commands, or it costs a
    full clone every run."""
    origin, head_sha = _origin_with_a_pull_ref(tmp_path)
    work = tmp_path / "repo"
    cloned: list[str] = []
    monkeypatch.setenv("GIT_TERMINAL_PROMPT", "0")
...
def test_the_marker_is_not_inside_the_tree_the_reviewers_write_to():
    """A marker under .git would be as forgeable as what it vouches for."""
    assert "/" not in review_pr_local.CLONE_MARKER
    assert not review_pr_local.CLONE_MARKER.startswith(".git")
...
def test_hooks_are_disarmed_on_every_run_not_just_at_clone_time(tmp_path, monkeypatch):
    """A hook planted between runs is removed by the next run's disarm.

    The other half of that sentence -- the run also unsets the config that
    ignores hooks -- is no longer this test's: a changed `.git/config` means
    the clone is replaced wholesale, which
    test_a_clone_whose_config_changed_is_replaced covers.
    """
    origin, head_sha = _origin_with_a_pull_ref(tmp_path)
    work = tmp_path / "repo"
    _git(tmp_path, "clone", "-q", str(origin), str(work))
