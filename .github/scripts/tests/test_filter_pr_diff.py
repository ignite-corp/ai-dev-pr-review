...
from aggregate_reviews import POLICY_SKIP_MARKER  # noqa: E402
...
from github_pr_support import REVIEW_MARKER, display_path  # noqa: E402
...
class TestMatcherSemantics:
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("*.[!c]", "x.o", True),
            ("*.[!c]", "x.c", False),
            # A negated class never matches the path separator.
            ("a[!x]b", "ayb", True),
            ("a[!x]b", "a/b", False),
            ("a[^x]b", "a/b", False),
            ("a[x-z]b", "ayb", True),
...
# git C-quotes a name with a control character; decoded, this one carries a
# real newline, which would otherwise add a line to every one-per-line list.
QUOTED_NEWLINE = (
    'diff --git "a/evil\\nInjected line" "b/evil\\nInjected line"\n'
    "new file mode 100644\n"
    "index 0000000..3333333\n"
    "--- /dev/null\n"
    '+++ "b/evil\\nInjected line"\n'
...
# A backtick needs no quoting from git, and closes a markdown code span.
BACKTICK_NAME = textwrap.dedent(
    """\
    diff --git a/tick`.pem b/tick`.pem
    new file mode 100644
    index 0000000..3333333
    --- /dev/null
    +++ b/tick`.pem
...
# ---------------------------------------------------------------------------
# 2a. Reported paths: matched raw, rendered single-line and code-span-safe
# ---------------------------------------------------------------------------


class TestReportedPaths:
    """A decoded path can hold anything; what is reported cannot break a list."""
    ...
    def test_display_path_is_single_line_and_code_span_safe(
        self, raw: str, shown: str
    ) -> None:
        assert display_path(raw) == shown
        assert len(shown.splitlines()) == 1
        assert "`" not in shown

    def test_display_path_leaves_an_ordinary_path_alone(self) -> None:
        path = "src/caf\u00e9 dir/file b.txt"
        assert display_path(path) == path

    def test_quoted_newline_header_decodes_to_a_real_newline(self) -> None:
        _, entries = split_diff(QUOTED_NEWLINE)
        assert entries[0].new_path == "evil\nInjected line"
    ...
    def test_the_rule_matches_the_raw_path_and_the_label_is_sanitized(self) -> None:
        ...
        assert kept == ""
        assert excluded == ["evil\\nInjected line"]
        assert excluded[0] != entries[0].new_path

    def test_a_backtick_in_the_name_is_replaced_in_the_label(self) -> None:
        kept, excluded = filter_diff(BACKTICK_NAME, parse_rules("*.pem\n"))
        assert kept == ""
        assert excluded == ["tick\u02cb.pem"]

    def test_a_rename_label_sanitizes_both_sides(self) -> None:
        kept, excluded = filter_diff(QUOTED_RENAME, parse_rules("old*\n"))
        assert kept == ""
        assert excluded == ["old\\nx -> new\\ny"]
...
class TestMain:
    ...
    def test_reported_paths_are_one_line_each_in_every_output(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        root = _workdir(
            tmp_path,
            monkeypatch,
            diff=QUOTED_NEWLINE + MODIFY + BACKTICK_NAME,
            rules="evil*\n*.pem\n",
        )
        filter_pr_diff.main()
        shown = ["evil\\nInjected line", "tick\u02cb.pem"]
        assert (root / "pr.diff").read_text(encoding="utf-8") == MODIFY
        outputs = _outputs(root)
        assert outputs["excluded_count"] == "2"
        # The heredoc carries exactly one line per excluded path.
        assert outputs["excluded_paths"].splitlines() == shown
        assert _result(root)["excluded_paths"] == shown
        # context.md keeps each path inside its own `- `...`` item; nothing
        # from a name lands on a line of its own.
        context = (root / "context.md").read_text(encoding="utf-8")
        items = [line for line in context.splitlines() if line.startswith("- ")]
        assert items == [f"- `{path}`" for path in shown]
        assert "Injected line" not in context.splitlines()
...
def _run_step(
    ...
    *,
    ...
    if pinned_script:
        ...
        # The pinned checkout ships the sibling module the script imports.
        for name in (SCRIPT.name, "github_pr_support.py"):
            shutil.copy(SCRIPT_DIR / name, target / name)
    output = tmp_path / "github_output"
    output.touch()
...
class TestPrepareStepExecution:
    ...
    def test_all_excluded_posts_the_skip_comment(self, tmp_path: Path) -> None:
        ...
        assert outputs["policy_skipped"] == "true"
        assert outputs["excluded_count"] == "2"
        # The stub logs one call per line, so a multi-line body spans lines:
        # the two markers the aggregate keys on lead the note, then the text.
        assert calls == [
            f"pr comment 35 --repo ignite-corp/ai-dev-pr-review --body {REVIEW_MARKER}",
            POLICY_SKIP_MARKER.format(n=2),
            "[i] Only policy-excluded files changed (2 file(s) matched"
            " .github/lens-ignore). Skipping AI review -- the aggregate verdict"
            " below lists the paths.",
        ]
        assert REVIEW_MARKER == "<!-- multi-llm-review -->"
        assert POLICY_SKIP_MARKER.format(n=2) == (
            "<!-- lens:skipped reason=policy-excluded-only files=2 -->"
        )
