...
def test_a_nonzero_exit_still_yields_a_verdict_the_log_holds(workdir, monkeypatch):
    """The argument for recovering from the log does not mention the exit code.

    test_a_nonzero_exit_fails_loudly_with_the_last_log_line covers a log with
    no verdict in it; this is the same exit with a complete one, which the
    gate discarded.
    """
    review = {"summary": "found one", "early_exit": False, "issues": []}
    _stub_cli(monkeypatch, 3, f"starting\n{json.dumps(review)}\nfatal: crashed\n")
    review_codex_local.main()
    assert _verdict(workdir)["summary"] == "found one"
