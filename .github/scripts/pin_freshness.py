...
Currency rule (MINOR tolerance)
    ...
    tolerance is defined at MINOR granularity on purpose.
...
def build_index(entries: Sequence[TagEntry]) -> tuple[dict[str, Version], dict[str, str]]:
    """Build ``tag -> version`` and ``sha -> tag`` maps from a tag listing.
...
def classify(pin: str, versions: dict[str, Version], by_sha: dict[str, str]) -> tuple[bool, str]:
    ...
    floating = _FLOATING_TAG_RE.match(pin)
    if floating:
        major = int(floating.group(1))
        if major == latest[0]:
            return True, "floating tag %s tracks latest %s" % (pin, _format(latest))
...
def main(argv: list[str] | None = None) -> int:
    ...
    versions, by_sha = build_index(load_entries(raw))
    if not versions:
    ...
    current, message = classify(args.pin, versions, by_sha)
    print(message)
