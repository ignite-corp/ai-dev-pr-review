...
def main(argv: list[str] | None = None) -> int:
    ...
    if not size_skipped:
        # Anything that goes wrong from here still has to reach the aggregate.
        # In Actions the aggregate job is gated on nothing precisely so that a
        # prepare that died still renders an explicit verdict (AT-2087); a
        # driver that raised out of main() instead left the PR with no verdict
        # at all -- the same defect through the local door.
        try:
            ...
            # Match what a failed prepare publishes in Actions: no head, no
            # author. The aggregate short-circuits on the failure before it
            # reads either, and an empty author selects the stricter
            # thresholds if anything downstream ever does.
            head_sha, pr_author = "", ""
            cwd = run_dir
