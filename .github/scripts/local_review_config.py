...
def read_config_file(path: Path) -> dict[str, str]:
    ...
    for number, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        if len(value) >= 2 and value[0] == value[-1] and value[0] in "\"'":
            ...
            # `CLAUDE_MODEL=claude-opus-5  # pinned` otherwise pins the model to
            # "claude-opus-5  # pinned", which reaches the CLI as a model id and
            # fails there, far from the line that caused it. Quote the value to
            # keep a literal `#`.
            value = _TRAILING_COMMENT_RE.sub("", value).rstrip()
...
def prompt_path_defaults() -> tuple[str, str]:
    ...
    for name in (SYSTEM_PROMPT_INPUT, CHECKLIST_INPUT):
        try:
            paths.append(inputs[name]["default"])
        except (KeyError, TypeError) as exc:
            raise ConfigError(
