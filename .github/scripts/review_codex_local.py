...
def review() -> None:
    ...
    # Tried on every path that has not already returned: after a malformed
    # direct write, after no file at all, and after a non-zero exit. Parity
    # does not decide this one -- the workflow gates on `[ -s ]` and never
    # validates the JSON, so it has no behaviour here to copy. What decides
    # it is that a complete verdict in the run log is the only usable answer
    # the run produced, and discarding it because something else went wrong
    # is the strictly worse half of the choice. That argument does not
    # depend on the exit code, so neither does this: the exit code decides
    # only which failure is reported when there is nothing to recover. The
    # Claude shim has never had such a gate.
    if extract_from_log():
        return
