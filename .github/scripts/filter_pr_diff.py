...
$GITHUB_OUTPUT, so the verdict comment lists them too. A reported path is
first passed through `display_path` -- control characters and line
separators become visible escapes, a backtick a lookalike -- because git's
C-quoted names can decode to anything and context.md is prompt input; the
rules always match the raw decoded path.
...
from github_pr_support import display_path
...
def _char_class(pattern: str, start: int) -> tuple[str, int] | None:
    ...
    # A negated class must not cross a path separator, as `*` and `?` do not.
...
class DiffEntry:
    ...
    @property
    def label(self) -> str:
        """The entry's name as reported; `paths` is what the rules match.

        Sanitized once here, so context.md, the JSON result, $GITHUB_OUTPUT
        and every comment built from them receive a single-line string that
        cannot close a code span or add a line to a one-per-line list.
        """
        if self.old_path and self.new_path and self.old_path != self.new_path:
            return f"{display_path(self.old_path)} -> {display_path(self.new_path)}"
        return display_path(self.new_path or self.old_path or "?")
