...
| `CLAUDE_FORCE_API` | _(unset)_ | Forces the Claude reviewer onto the billed `ANTHROPIC_API_KEY` path. The caller collapses this var to `true`/`false` and passes it to the `claude-review` composite, which then passes the API key **and blanks** `CLAUDE_CODE_OAUTH_TOKEN` so the CLI cannot prefer OAuth. Only the literal `true` switches; unset or any other value keeps the OAuth-preferred default. Ops-managed and normally absent — `.github/scripts/switch_claude_auth.py` creates it as an `ignite-corp` organization variable (visibility `all`) when a review hits the Claude subscription usage limit, and deletes it org-wide together with `CLAUDE_FORCE_API_UNTIL` once that limit resets. Absent is the healthy steady state; consumers should not set it by hand. |
