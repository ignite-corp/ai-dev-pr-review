...
Wrapper version alignment: this repo and the pilot wrapper `ignite-pilot-org/ai-dev-pr-review-wrapper` keep their release versions in MAJOR.MINOR lockstep; PATCH moves independently per repo. Current pairing: base `v1.8.1` ↔ wrapper `v1.8.0`. When either repo needs a MINOR (or MAJOR) bump, the other cuts a matching alignment release — content-identical if it has no changes. Both repos auto-float their `v1` major tag via `move-major-tag.yml` on release publish, so `@v1` consumers need no action.
