...
| `CLAUDE_FORCE_API` | _(미설정)_ | Claude 리뷰어가 어떤 인증 경로를 탈지 고르는 불리언 스위치. 값은 리터럴 문자열 `true` 하나뿐이며, 자격증명을 담지 않습니다. 호출부가 이 변수를 `true`/`false`로 접어 `claude-review` composite에 `force_api`로 전달하면, composite가 `ANTHROPIC_API_KEY` **시크릿**을 전달하면서 `CLAUDE_CODE_OAUTH_TOKEN`을 **비워** CLI가 OAuth를 우선하지 못하게 함. 키 자체는 그 시크릿에 그대로 있으며(위 시크릿 표 참고) 이 변수로 복사되지 않습니다. 설정되어 있는 동안 Claude 리뷰는 Pro/Max 구독이 아니라 Anthropic API 계정에 과금되며, `CLAUDE_CODE_OAUTH_TOKEN`만 구성한 레포는 이 경로에서 쓸 자격증명이 남지 않습니다 — Claude 리뷰어를 쓴다면 `ANTHROPIC_API_KEY`를 함께 구성해 두세요. 미설정이거나 그 외의 값이면 OAuth 우선 기본 동작 유지. 운영(ops)이 관리하며 평소에는 존재하지 않음 — `.github/scripts/switch_claude_auth.py`가 리뷰 중 Claude 구독 사용량 한도를 감지하면 이 플래그를 `ignite-corp` 조직 변수(visibility `all`)로 기록하고, 한도가 초기화되면 `CLAUDE_FORCE_API_UNTIL`과 함께 조직 전체에서 삭제함. 부재가 정상 상태이므로 소비자가 직접 설정하지 마세요. |
