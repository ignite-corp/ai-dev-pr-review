...
| `CLAUDE_FORCE_API` | _(미설정)_ | Claude 리뷰어를 과금되는 `ANTHROPIC_API_KEY` 경로로 강제함. 호출부가 이 변수를 `true`/`false`로 접어 `claude-review` composite에 전달하면, composite가 API 키를 전달하면서 `CLAUDE_CODE_OAUTH_TOKEN`을 **비워** CLI가 OAuth를 우선하지 못하게 함. 리터럴 `true`일 때만 전환되며, 미설정이거나 그 외의 값이면 OAuth 우선 기본 동작 유지. 운영(ops)이 관리하며 평소에는 존재하지 않음 — `.github/scripts/switch_claude_auth.py`가 리뷰 중 Claude 구독 사용량 한도를 감지하면 `ignite-corp` 조직 변수(visibility `all`)로 생성하고, 한도가 초기화되면 `CLAUDE_FORCE_API_UNTIL`과 함께 조직 전체에서 삭제함. 부재가 정상 상태이므로 소비자가 직접 설정하지 마세요. |
